...
namespace Activity_01_Lab
{
    public partial class Form1 : Form
    {
        ...
        string status;
        string Hobby;
        string strconnection = "Data Source=AUMC-LAB3-12\\SQLEXPRESS;Initial Catalog=CustomerDB;Integrated Security=True";
        public Form1()
        {
            InitializeComponent();
        }
        ...
        {
            // TODO: This line of code loads data into the 'customerDBDataSet7.Customer' table. You can move, or remove it, as needed.
            this.customerTableAdapter1.Fill(this.customerDBDataSet7.Customer);
        ...
        private void btn_add_Click(object sender, EventArgs e)
        {
            if (form_data_validation())
            {
                try
                {
                    ...
                    SqlConnection objconnection = new SqlConnection(strconnection);
                    objconnection.Open();
                    string strcommand = $"INSERT INTO Customer VALUES ({txt_cname.Text},{combo_country.Text},{Gender},{status},{Hobby})";
                    SqlCommand objCommand = new SqlCommand(strcommand, objconnection);
        ...
        private void btn_delete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlConnection objconnection = new SqlConnection(strconnection);
                objconnection.Open();
                string strcommand = $"DELETE FROM Customer WHERE Customer_Name = {txt_cname.Text}";
                SqlCommand objCommand = new SqlCommand(strcommand, objconnection);
                int rowsaffected = objCommand.ExecuteNonQuery();
                if (rowsaffected > 0)
                {
        ...
        private void displayCustomer(string strCustomer)
        {
            ...
            objConnection.Open();
            // Fire a Command
            string strCommand = "Select * From Customer where CustomerName = '" + strCustomer + "'";
            SqlCommand objCommand = new SqlCommand(strCommand, objConnection);
            // Bind Data with UI
            DataSet objDataSet = new DataSet();
            SqlDataAdapter objAdapter = new SqlDataAdapter(objCommand);
