        ...
        private void btn_add_Click(object sender, EventArgs e)
        {
            if (form_data_validation())
            {
                try
                {
                    ...
                    {
                        objconnection.Open();
                        string strcommand = "INSERT INTO Customer (CustomerName, Country, Gender, Hobbies, Status) VALUES (@CustomerName, @Country, @Gender, @Hobby, @Status)";
                        using (SqlCommand objCommand = new SqlCommand(strcommand, objconnection))
                        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (form_data_validation())
            {
                try
                {
                    ...
                    {
                        objconnection.Open();
                        string strcommand = "UPDATE Customer SET Country = @Country, Gender = @Gender, Hobbies = @Hobby, Status = @Status WHERE CustomerName = @CustomerName";
                        using (SqlCommand objCommand = new SqlCommand(strcommand, objconnection))
                        {
                            objCommand.Parameters.AddWithValue("@CustomerName", txt_cname.Text);
                            objCommand.Parameters.AddWithValue("@Country", combo_country.Text);
